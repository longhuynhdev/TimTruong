...
export function LogoCloud({
	...
	const cellBase =
		"w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 rounded-xl transition-all duration-300";
	...
	const iconStyle =
		"w-full h-full transform group-hover:scale-110 transition-transform duration-300 flex items-center justify-center [&>svg]:w-full [&>svg]:h-full [&>img]:w-full [&>img]:h-full [&>img]:object-contain";

	return (
		<section
			className={cn(
				"w-full py-16 md:py-20 lg:py-24 bg-background overflow-hidden",
				className,
			)}
		>
			<div className="w-full px-4">
				...
				<div className="text-center mb-10 md:mb-14 lg:mb-16">
					{/* Serif family for the slanted look: it has a real italic
					    (ui-serif "New York" on Apple supports a true light italic; Georgia
					    elsewhere), so iOS renders it correctly instead of a broken
					    browser-synthesized oblique (Outfit/sans has no true italic).
					    Softened color + tight leading keep it delicate, not heavy. */}
							...
							{logos.length > 6 && (
								<>
									...
									{logos.slice(6, 12).length < 6 &&
										Array.from({ length: 6 - logos.slice(6, 12).length }).map(
											(_, i) => (
												<div
													key={`desktop-r2-empty-${i}`}
							...
							{logos.length > 12 && (
								<>
									...
									{logos.slice(12, 16).length < 4 &&
										Array.from({ length: 4 - logos.slice(12, 16).length }).map(
											(_, i) => (
												<div
													key={`desktop-r3-empty-${i}`}
													className={emptyCellStyle}
												/>
											),
										)}
